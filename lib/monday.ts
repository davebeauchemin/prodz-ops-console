...
/** Parse response as JSON; on failure throw clear error with response snippet. */
...
/**
 * Parse date from Monday date column value (JSON: { date: "YYYY-MM-DD" }).
 */
function parseDateFromValue(value: string): string | null {
  try {
    ...
    if (typeof date === "string") {
      return date.split("T")[0] ?? date; // "2024-03-10" or "2024-03-10T00:00:00"
    }
    return null;
  } catch {
    return null;
...
/**
 * Parse dropdown/labels value. text field has display value, or parse value JSON.
 */
...
/**
 * Parse Actions column - can be multi-select. Returns array of action labels.
 * When API returns cv.text as comma-separated (e.g. "Follow-up - SMS Sent, Follow-up - Email Sent"),
 * split so each label can match FUP_ACTIONS individually.
 */
...
/** Parse item and include date; no date filter. Returns null if no valid date. */
...
/**
 * Fetch board columns to get id -> title mapping.
 * ColumnValue does not have "title"; we need column ids to identify Date, Setter, etc.
 */
...
/**
 * Fetch items from Monday.com Setters Reports board for a given date.
 * Filters to "Daily Report" group and items matching the date.
 */
export async function fetchItemsByDate(
  ...
  do {
    ...
    if (cursor) {
      const page = (json as { data?: { next_items_page: { cursor: string | null; items: MondayItemRaw[] } } })
...
/**
 * Fetch all Daily Report items in a date range. Fetches the board once and groups by date.
 * Returns Map<date, MondayItem[]> keyed by YYYY-MM-DD.
 */
